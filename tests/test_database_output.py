...
def test_copy_images_copies_only_webp(tmp_path: Path) -> None:
    output_dir = tmp_path / "Output"
    images_dir = tmp_path / "Images"
    output_dir.mkdir()
    (output_dir / "a.webp").write_bytes(b"img-a")
    (output_dir / "b.webp").write_bytes(b"img-b")
    (output_dir / "a.txt").write_text("x", encoding="utf-8")
    (output_dir / "c.pdf").write_bytes(b"skip")

    count, dest = copy_images(output_dir, images_dir)

    assert count == 2
    assert sorted(p.name for p in dest.iterdir()) == ["a.webp", "b.webp"]
    assert (dest / "a.webp").read_bytes() == b"img-a"
